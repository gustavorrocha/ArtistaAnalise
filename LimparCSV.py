...
def limpar_dataframes(lista_caminhos, ignorar_variacoes = True):
    """Recebe uma lista com o caminho dos arquivos e retorna uma lista de dataframes 'limpos'

    :param lista_caminhos: Lista com os caminhos dos arquivos .csv
    :type lista_caminhos: list[str]
    :param ignorar_variacoes: Caso o usuário deseje manter os álbums que possuem variações deve setar como False, o padrão é True
    :type ignorar_variacoes: bool, opcional
    :return: Lista de dataframes limpos para uso
    :rtype: pandas.DataFrame
    """

    if not isinstance(lista_caminhos, list):
        # Levanta um erro caso o tipo de 'lista_caminhos' seja inválido
        raise TypeError("A lista dos caminhos deve ser do tipo 'list'!")

    if not isinstance(ignorar_variacoes, bool):
        # Levanta um erro caso o tipo de 'ignorar_variacoes' seja inválido
        raise TypeError("O atributo 'ignorar_variacoes' deve ser do tipo 'bool'!")
    ...
    for caminho in lista_caminhos:
        ...
        try: 
            ...
            print(f"O arquivo em {caminho} não possui formatação compatível com a função!")
            sys.exit(2)
...
def eh_variacao(nome):
    """Retorna se o nome do álbum revela que ele é uma variação
    ...
    if not isinstance(nome, str):
        # Levanta um erro caso o tipo de 'nome' seja inválido
        raise TypeError("A entrada da função deve ser to tipo 'str'!")
...
def eliminiar_duplicata_df(df):
    """Recebe um DataFrame com todos os 'álbuns' que possuem o mesmo nome e retorna o que tiver mais músicas
    ...
    if not isinstance(df, pd.DataFrame):
        # Levanta um erro caso o tipo de entrada seja inválido
        raise TypeError("A entrada da função deve um DataFrame!")
    ...
    try:
        ...
        return(f"O DataFrame não possui formatação compatível com a função!")
...
def formatar_df_album(df_album):
    """Altera a formatação do dataframe de um álbum
    ...
    if not isinstance(df_album, pd.DataFrame):
        # Levanta um erro caso o tipo de entrada seja inválido
        raise TypeError("A entrada da função deve um DataFrame!")
    ...
    try:
        ...
        return "O DataFrame não possui formatação compatível com a função!"
...
def juntar_dataframes(caminho_letra, caminho_dados):
    ...
    try:
        ...
        return "Os DataFrames de entrada não possuem formato compatível!"

    return df # Retorna o que foi obtido


ARTISTA = "Adele"
juntar_dataframes(f"./Infos/Letras - {ARTISTA}.csv", f"./Infos/Dados - {ARTISTA}.csv")
