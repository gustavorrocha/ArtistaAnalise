...
def encontrar_albums(nome_artista, tipos_album):
    """Encontra determinados tipos de álbum de um artista
    ...
    if set(tipos_album).issubset(possiveis_tipos) == False: 
        raise ValueError("Tipos de álbum inválidos!")
    ...
    if len(albums_lista) < 1:
        # Caso não seja encontrado nenhum álbum, levanta um erro
        raise ValueError("Álbums do artista não encontrados!") 

    return albums_lista # Retorna a lista de álbums obtida


def obter_dados(nome_artista, tipos_album):
    """Obtém os dados de cada música
    ...
    try:
        ...
        for album in albums:
            ...
            for musica in musicas.get("tracks"):
                ...
                musicas_dados.append({"album_id": album.get("id"), "album": album["nome"], "musicas_album": len(musicas_album_ids), 
                "num_album": num_track, "nome": musica_nome, "data": album["data"], "artistas": musica_nomes_artistas, 
                "popularidade": musica_popularidade, "letra_explicita": musica_explicita, "duracao": musica_duracao, "volume": musica_volume, 
                "bpm": musica_bpm, "energia": musica_energia, "dancabilidade": musica_dancabilidade, "vivacidade": musica_vivacidade, 
                "fala": musica_fala, "acustica": musica_acustica, "instrumentalidade": musica_instrumentalidade, 
                "valencia": musica_valencia, "chave": musica_chave, "modo": musica_modo, "assinatura_tempo": musica_assinatura_tempo}) 
    
    except ValueError as erro:
        # Interrompe a execução se a função anterior levantar um erro
        print(erro)
        sys.exit(2)
...
def criar_df_dados(nome_artista, tipos_album):
    """Cria um DataFrame com os dados das músicas
...
try:
    ...
    # Caso as credenciais sejam inválidas imprime um erro e interrompe a execução
    print("Credenciais inválidas, erro na autenticação!")
    sys.exit()

sp = spotipy.Spotify(client_credentials_manager=credenciais)

print(criar_df_dados(1, ["album"]))
